...
const char *adaptive_phase_name(adaptive_phase_t phase) {
    switch (phase) {
        case ADAPTIVE_PHASE_BASELINE:  return "BASELINE";
        case ADAPTIVE_PHASE_PROBING:   return "PROBING";
        case ADAPTIVE_PHASE_STEADY:    return "STEADY";
        case ADAPTIVE_PHASE_BACKOFF:   return "BACKOFF";
        case ADAPTIVE_PHASE_SETTLING:  return "SETTLING";
        case ADAPTIVE_PHASE_CONVERGED: return "CONVERGED";
...
bool adaptive_tick(adaptive_controller_t *ctrl) {
    ...
    int64_t bytes = atomic_load_explicit(&ctrl->sample_bytes, memory_order_relaxed);
    ...
    int calls = atomic_load_explicit(&ctrl->submit_calls, memory_order_relaxed);
    int sqes = atomic_load_explicit(&ctrl->sqes_submitted, memory_order_relaxed);
    if (calls > 0) {
    ...
    if (p99_ms >= 0) {
        window_add(ctrl->p99_window, &ctrl->p99_head, &ctrl->p99_count,
                   ADAPTIVE_P99_WINDOW, p99_ms);
    ...
    int batch_threshold = atomic_load_explicit(&ctrl->current_batch_threshold, memory_order_relaxed);
    if (sqe_ratio > 0) {
        if (sqe_ratio < ADAPTIVE_TARGET_SQE_RATIO && batch_threshold < ADAPTIVE_MAX_BATCH_THRESHOLD) {
            batch_threshold++;
            atomic_store_explicit(&ctrl->current_batch_threshold, batch_threshold, memory_order_relaxed);
            params_changed = true;
    ...
    int in_flight_limit = atomic_load_explicit(&ctrl->current_in_flight_limit, memory_order_relaxed);

    /* Calculate efficiency ratio: change in throughput per change in in-flight */
    double efficiency_ratio = 0.0;
    if (ctrl->prev_in_flight_limit > 0 &&
        in_flight_limit != ctrl->prev_in_flight_limit) {
