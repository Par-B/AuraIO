...
pub struct RingStats {
    inner: auraio_sys::auraio_ring_stats_t,
}

impl RingStats {
    /// Create a new ring stats snapshot (internal use)
    pub(crate) fn new(inner: auraio_sys::auraio_ring_stats_t) -> Self {
        Self { inner }
    }
    ...
    pub fn aimd_phase_name(&self) -> &'static str {
        unsafe {
            let ptr = auraio_sys::auraio_phase_name(self.inner.aimd_phase);
            if ptr.is_null() {
                "unknown"
            } else {
...
pub struct Histogram {
    inner: auraio_sys::auraio_histogram_t,
}

impl Histogram {
    /// Number of histogram buckets
    pub const BUCKET_COUNT: usize = auraio_sys::AURAIO_HISTOGRAM_BUCKETS as usize;

    /// Bucket width in microseconds
    pub const BUCKET_WIDTH_US: i32 = auraio_sys::AURAIO_HISTOGRAM_BUCKET_WIDTH_US as i32;

    /// Create a new histogram snapshot (internal use)
    pub(crate) fn new(inner: auraio_sys::auraio_histogram_t) -> Self {
        Self { inner }
    }
...
pub struct BufferStats {
    inner: auraio_sys::auraio_buffer_stats_t,
}

impl BufferStats {
    /// Create a new buffer stats snapshot (internal use)
    pub(crate) fn new(inner: auraio_sys::auraio_buffer_stats_t) -> Self {
        Self { inner }
    }
