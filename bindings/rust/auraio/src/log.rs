...
//!
//! ```no_run
//! use auraio::{LogLevel, set_log_handler, clear_log_handler, log_emit};
//!
...
impl LogLevel {
    ...
    fn from_c(level: c_int) -> Self {
        match level as u32 {
            auraio_sys::AURAIO_LOG_ERR => LogLevel::Error,
            auraio_sys::AURAIO_LOG_WARN => LogLevel::Warning,
            auraio_sys::AURAIO_LOG_NOTICE => LogLevel::Notice,
            auraio_sys::AURAIO_LOG_INFO => LogLevel::Info,
            _ => LogLevel::Debug,
        }
    }
...
///
/// ```no_run
/// auraio::set_log_handler(|level, msg| {
///     eprintln!("[{}] {}", level, msg);
/// });
/// ```
    ...
    F: Fn(LogLevel, &str) + Send + 'static,
{
    ...
    unsafe {
        auraio_sys::auraio_set_log_handler(Some(log_trampoline), std::ptr::null_mut());
    }
}
...
pub fn clear_log_handler() {
    unsafe {
        auraio_sys::auraio_set_log_handler(None, std::ptr::null_mut());
    }
    let mut guard = LOG_HANDLER.lock().unwrap_or_else(|e| e.into_inner());
    *guard = None;
}
...
pub fn log_emit(level: LogLevel, msg: &str) {
    if let Ok(c_msg) = CString::new(msg) {
        ...
        unsafe {
            auraio_sys::auraio_log_emit(level.to_c(), fmt, c_msg.as_ptr());
        }
    }
}
