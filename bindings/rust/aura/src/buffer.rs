...
/// (equivalent to the C inline function aura_buf)
fn make_buf(ptr: *mut std::ffi::c_void) -> aura_sys::aura_buf_t {
    let mut buf: aura_sys::aura_buf_t = unsafe { std::mem::zeroed() };
    buf.type_ = aura_sys::aura_buf_type_t_AURA_BUF_UNREGISTERED;
    buf.u.ptr = ptr;
    buf
}
...
impl BufferRef {
    ...
    pub(crate) fn as_raw(&self) -> aura_sys::aura_buf_t {
        self.inner
    }
}
