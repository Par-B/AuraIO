 ...
 * - log.c (0% → covered)
 * - auraio.c error paths, drain, vectored I/O, request introspection
...
static void basic_cb(auraio_request_t *req, ssize_t result, void *ud) {
    (void)req;
    (void)ud;
...
TEST(request_fd_null) {
    int fd = auraio_request_fd(NULL);
    assert(fd == -1);
    assert(errno == EINVAL);
}

TEST(request_user_data_null) {
    void *ud = auraio_request_user_data(NULL);
    assert(ud == NULL);
    assert(errno == EINVAL);
}

TEST(request_pending_null) {
    assert(auraio_request_pending(NULL) == false);
}
...
TEST(request_introspection_during_io) {
    ...
    auraio_request_t *req =
        auraio_read(engine, test_fd, auraio_buf(buf), 4096, 0, basic_cb, &my_sentinel);
    assert(req);

    /* Before completion, request should be pending */
    assert(auraio_request_pending(req) == true);
    assert(auraio_request_fd(req) == test_fd);
    assert(auraio_request_user_data(req) == &my_sentinel);
...
TEST(poll_fd_null_engine) {
    int fd = auraio_get_poll_fd(NULL);
    assert(fd == -1);
}
...
TEST(poll_fd_valid) {
    auraio_engine_t *engine = make_engine(1, 32);
    assert(engine);
    int fd = auraio_get_poll_fd(engine);
    assert(fd >= 0);
    auraio_destroy(engine);
}
...
TEST(drain_no_pending) {
    ...
    int rc = auraio_drain(engine, 1000);
    assert(rc == 0);
...
TEST(drain_with_pending_io) {
    ...
    cb_called = 0;
    auraio_request_t *req = auraio_read(engine, test_fd, auraio_buf(buf), 4096, 0, basic_cb, NULL);
    assert(req);

    int rc = auraio_drain(engine, 5000);
    assert(rc >= 1);
    assert(cb_called == 1);
...
TEST(readv_basic) {
    ...
    cb_called = 0;
    auraio_request_t *req = auraio_readv(engine, test_fd, iov, 2, 0, basic_cb, NULL);
    assert(req);
    auraio_wait(engine, 1000);
    assert(cb_called == 1);
...
TEST(writev_basic) {
    ...
    cb_called = 0;
    auraio_request_t *req = auraio_writev(engine, test_fd, iov, 2, 0, basic_cb, NULL);
    assert(req);
    auraio_wait(engine, 1000);
    assert(cb_called == 1);
...
TEST(readv_null_args) {
    ...
    /* NULL engine */
    assert(auraio_readv(NULL, 0, NULL, 0, 0, NULL, NULL) == NULL);
    assert(errno == EINVAL);

    /* NULL iov */
    assert(auraio_readv(engine, test_fd, NULL, 1, 0, NULL, NULL) == NULL);
    assert(errno == EINVAL);

    /* iovcnt = 0 */
    struct iovec iov = { .iov_base = NULL, .iov_len = 0 };
    assert(auraio_readv(engine, test_fd, &iov, 0, 0, NULL, NULL) == NULL);
    assert(errno == EINVAL);
...
TEST(writev_null_args) {
    ...
    assert(auraio_writev(NULL, 0, NULL, 0, 0, NULL, NULL) == NULL);
    assert(errno == EINVAL);

    assert(auraio_writev(engine, test_fd, NULL, 1, 0, NULL, NULL) == NULL);
    assert(errno == EINVAL);
...
TEST(fsync_basic) {
    ...
    cb_called = 0;
    auraio_request_t *req = auraio_fsync(engine, test_fd, AURAIO_FSYNC_DEFAULT, basic_cb, NULL);
    assert(req);
    auraio_wait(engine, 1000);
    assert(cb_called == 1);
    assert(cb_result == 0);
...
TEST(fsync_datasync) {
    ...
    cb_called = 0;
    auraio_request_t *req = auraio_fsync(engine, test_fd, AURAIO_FSYNC_DATASYNC, basic_cb, NULL);
    assert(req);
    auraio_wait(engine, 1000);
    assert(cb_called == 1);
    assert(cb_result == 0);
...
TEST(fsync_null_callback) {
    ...
    auraio_request_t *req = auraio_fsync(engine, test_fd, AURAIO_FSYNC_DEFAULT, NULL, NULL);
    assert(req);
    auraio_drain(engine, 1000);
...
TEST(fsync_null_args) {
    assert(auraio_fsync(NULL, 0, AURAIO_FSYNC_DEFAULT, NULL, NULL) == NULL);
    assert(errno == EINVAL);
    ...
    /* Negative fd */
    assert(auraio_fsync(engine, -1, AURAIO_FSYNC_DEFAULT, NULL, NULL) == NULL);
    assert(errno == EINVAL);
...
TEST(read_null_engine) {
    char buf[64];
    assert(auraio_read(NULL, 0, auraio_buf(buf), 64, 0, NULL, NULL) == NULL);
    assert(errno == EINVAL);
}
...
TEST(read_negative_fd) {
    ...
    assert(engine);
    char buf[64];
    assert(auraio_read(engine, -1, auraio_buf(buf), 64, 0, NULL, NULL) == NULL);
    assert(errno == EINVAL);
    auraio_destroy(engine);
}
...
TEST(read_zero_length) {
    ...
    assert(engine);
    char buf[64];
    assert(auraio_read(engine, 0, auraio_buf(buf), 0, 0, NULL, NULL) == NULL);
    assert(errno == EINVAL);
    auraio_destroy(engine);
}
...
TEST(read_null_buf_ptr) {
    auraio_engine_t *engine = make_engine(1, 32);
    assert(engine);
    assert(auraio_read(engine, 0, auraio_buf(NULL), 64, 0, NULL, NULL) == NULL);
    assert(errno == EINVAL);
    auraio_destroy(engine);
}
...
TEST(write_null_engine) {
    char buf[64];
    assert(auraio_write(NULL, 0, auraio_buf(buf), 64, 0, NULL, NULL) == NULL);
    assert(errno == EINVAL);
}
...
TEST(write_negative_fd) {
    ...
    assert(engine);
    char buf[64];
    assert(auraio_write(engine, -1, auraio_buf(buf), 64, 0, NULL, NULL) == NULL);
    assert(errno == EINVAL);
    auraio_destroy(engine);
}
...
TEST(write_zero_length) {
    ...
    assert(engine);
    char buf[64];
    assert(auraio_write(engine, 0, auraio_buf(buf), 0, 0, NULL, NULL) == NULL);
    assert(errno == EINVAL);
    auraio_destroy(engine);
}
...
TEST(read_null_callback) {
    ...
    auraio_request_t *req = auraio_read(engine, test_fd, auraio_buf(buf), 4096, 0, NULL, NULL);
    assert(req);
    auraio_drain(engine, 1000);
...
TEST(write_null_callback) {
    ...
    auraio_request_t *req = auraio_write(engine, test_fd, auraio_buf(buf), 4096, 0, NULL, NULL);
    assert(req);
    auraio_drain(engine, 1000);
...
TEST(cancel_null_args) {
    assert(auraio_cancel(NULL, NULL) == -1);
    assert(errno == EINVAL);

    auraio_engine_t *engine = make_engine(1, 32);
    assert(engine);
    assert(auraio_cancel(engine, NULL) == -1);
    assert(errno == EINVAL);
    auraio_destroy(engine);
}
...
TEST(register_buffers_null_args) {
    assert(auraio_register_buffers(NULL, NULL, 0) == -1);
    assert(errno == EINVAL);

    auraio_engine_t *engine = make_engine(1, 32);
    assert(engine);

    assert(auraio_register_buffers(engine, NULL, 1) == -1);
    assert(errno == EINVAL);

    struct iovec iov = { .iov_base = NULL, .iov_len = 4096 };
    assert(auraio_register_buffers(engine, &iov, 0) == -1);
    assert(errno == EINVAL);

    assert(auraio_register_buffers(engine, &iov, -1) == -1);
    assert(errno == EINVAL);
...
TEST(read_fixed_no_registration) {
    ...
    auraio_buf_t fbuf = auraio_buf_fixed(0, 0);
    auraio_request_t *req = auraio_read(engine, 0, fbuf, 4096, 0, NULL, NULL);
    assert(req == NULL);
    assert(errno == ENOENT);
...
TEST(write_fixed_no_registration) {
    ...
    auraio_buf_t fbuf = auraio_buf_fixed(0, 0);
    auraio_request_t *req = auraio_write(engine, 0, fbuf, 4096, 0, NULL, NULL);
    assert(req == NULL);
    assert(errno == ENOENT);
...
TEST(register_buffers_and_use) {
    ...
    cb_called = 0;
    auraio_request_t *req =
        auraio_read(engine, test_fd, auraio_buf_fixed(0, 0), 4096, 0, basic_cb, NULL);
    assert(req);
    auraio_wait(engine, 1000);
    assert(cb_called == 1);
    assert(cb_result == 4096);
    ...
    req = auraio_read(engine, test_fd, auraio_buf_fixed(99, 0), 4096, 0, NULL, NULL);
    assert(req == NULL);
    assert(errno == EINVAL);

    /* Overflow: offset beyond buffer length */
    req = auraio_read(engine, test_fd, auraio_buf_fixed(0, 8192), 4096, 0, NULL, NULL);
    assert(req == NULL);
    assert(errno == EOVERFLOW);

    /* Overflow: length exceeds remaining space */
    req = auraio_read(engine, test_fd, auraio_buf_fixed(0, 0), 8192, 0, NULL, NULL);
    assert(req == NULL);
    assert(errno == EOVERFLOW);
    ...
    cb_called = 0;
    req = auraio_write(engine, test_fd, auraio_buf_fixed(0, 0), 4096, 0, basic_cb, NULL);
    assert(req);
    auraio_wait(engine, 1000);
    assert(cb_called == 1);
    assert(cb_result == 4096);
    ...
    /* Unregister */
    rc = auraio_unregister_buffers(engine);
    assert(rc == 0);

    /* Post-unregister: fixed buffers should fail with ENOENT */
    req = auraio_read(engine, test_fd, auraio_buf_fixed(0, 0), 4096, 0, NULL, NULL);
    assert(req == NULL);
    assert(errno == ENOENT);
...
TEST(unregister_buffers_not_registered) {
    ...
    int rc = auraio_unregister_buffers(engine);
...
TEST(register_files_null_args) {
    assert(auraio_register_files(NULL, NULL, 0) == -1);
    assert(errno == EINVAL);

    auraio_engine_t *engine = make_engine(1, 32);
    assert(engine);

    assert(auraio_register_files(engine, NULL, 1) == -1);
    assert(errno == EINVAL);

    int fd = 0;
    assert(auraio_register_files(engine, &fd, 0) == -1);
    assert(errno == EINVAL);

    assert(auraio_register_files(engine, &fd, -1) == -1);
    assert(errno == EINVAL);
...
TEST(register_files_and_io) {
    ...
    int rc = auraio_register_files(engine, &test_fd, 1);
    assert(rc == 0);

    /* Double registration should fail */
    rc = auraio_register_files(engine, &test_fd, 1);
    assert(rc == -1);
    assert(errno == EBUSY);
    ...
    rc = auraio_unregister_files(engine);
    assert(rc == 0);
...
TEST(update_file_null_args) {
    assert(auraio_update_file(NULL, 0, -1) == -1);
    assert(errno == EINVAL);
    ...
    /* Negative index */
    assert(auraio_update_file(engine, -1, 0) == -1);
    assert(errno == EINVAL);

    /* No files registered */
    assert(auraio_update_file(engine, 0, -1) == -1);
    assert(errno == ENOENT);
...
TEST(update_file_out_of_range) {
    ...
    /* Index beyond count */
    rc = auraio_update_file(engine, 99, -1);
    assert(rc == -1);
    assert(errno == EINVAL);
...
TEST(unregister_files_not_registered) {
    auraio_engine_t *engine = make_engine(1, 32);
    assert(engine);
    int rc = auraio_unregister_files(engine);
    (void)rc;
    auraio_destroy(engine);
}
...
TEST(request_unregister_buffers_null) {
    assert(auraio_request_unregister_buffers(NULL) == -1);
    assert(errno == EINVAL);
}

TEST(request_unregister_files_null) {
    assert(auraio_request_unregister_files(NULL) == -1);
    assert(errno == EINVAL);
}
...
TEST(deferred_unregister_buffers) {
    ...
    rc = auraio_request_unregister_buffers(engine);
    assert(rc == 0);
    ...
    auraio_request_t *req =
        auraio_read(engine, test_fd, auraio_buf_fixed(0, 0), 4096, 0, NULL, NULL);
    assert(req == NULL);
...
TEST(deferred_unregister_files) {
    ...
    rc = auraio_request_unregister_files(engine);
    assert(rc == 0);
...
TEST(version_int) {
    int vi = auraio_version_int();
    assert(vi > 0);
    assert(vi == AURAIO_VERSION);
}
...
TEST(buffer_alloc_free_multiple_sizes) {
    ...
    for (int i = 0; i < 6; i++) {
        bufs[i] = auraio_buffer_alloc(engine, sizes[i]);
        assert(bufs[i]);
    ...
    for (int i = 0; i < 6; i++) {
        auraio_buffer_free(engine, bufs[i], sizes[i]);
    }
...
TEST(buffer_alloc_null_engine) {
    void *buf = auraio_buffer_alloc(NULL, 4096);
    assert(buf == NULL);
}
...
TEST(buffer_free_null) {
    ...
    auraio_buffer_free(engine, NULL, 4096);
    auraio_destroy(engine);
}
...
static void *buffer_thread(void *arg) {
    ...
    for (int i = 0; i < ctx->iterations; i++) {
        void *buf = auraio_buffer_alloc(ctx->engine, 4096);
        assert(buf);
        memset(buf, 0, 4096);
...
TEST(buffer_pool_huge_alloc) {
    ...
    void *buf = auraio_buffer_alloc(engine, 256 * 1024 * 1024);
    /* Buffer should be NULL for oversized request */
    assert(buf == NULL);

    /* Try allocating exactly at the max (128MB) */
    buf = auraio_buffer_alloc(engine, 128 * 1024 * 1024);
    if (buf) {
        auraio_buffer_free(engine, buf, 128 * 1024 * 1024);
    }
...
#define NUM_BUFS 600
    ...
    for (int i = 0; i < NUM_BUFS; i++) {
        bufs[i] = auraio_buffer_alloc(engine, 8192);
        assert(bufs[i]);
    }
    for (int i = 0; i < NUM_BUFS; i++) {
        auraio_buffer_free(engine, bufs[i], 8192);
    }

    /* Third round: Force the "no metadata slots" path by exhausting slots */
    for (int i = 0; i < NUM_BUFS; i++) {
        bufs[i] = auraio_buffer_alloc(engine, 16384);
        assert(bufs[i]);
    }
    for (int i = 0; i < NUM_BUFS; i++) {
        auraio_buffer_free(engine, bufs[i], 16384);
    }
...
TEST(buffer_pool_cross_thread_reuse) {
    ...
    void *buf1 = auraio_buffer_alloc(engine, 8192);
    assert(buf1);
    auraio_buffer_free(engine, buf1, 8192);
...
TEST(buffer_pool_size_classes) {
    ...
    for (size_t i = 0; i < sizeof(sizes) / sizeof(sizes[0]); i++) {
        void *buf = auraio_buffer_alloc(engine, sizes[i]);
        if (buf) { /* May be NULL for very large sizes */
            auraio_buffer_free(engine, buf, sizes[i]);
        }
    }
...
TEST(ring_request_pool_exhaustion) {
    ...
    int dummy_idx;
    auraio_request_t *req = ring_get_request(&ctx, &dummy_idx);
    assert(req == NULL);
...
TEST(ring_wait_nonblocking) {
    ...
    /* Submit an operation */
    int op_idx;
    auraio_request_t *req = ring_get_request(&ctx, &op_idx);
    assert(req != NULL);
...
TEST(buffer_pool_mixed_sizes) {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        bufs[i] = auraio_buffer_alloc(engine, size);
        assert(bufs[i]);
    }
    ...
    for (int i = 99; i >= 0; i--) {
        size_t size = sizes[i % 6];
        auraio_buffer_free(engine, bufs[i], size);
    }
...
/* ============================================================================
 * auraio_buf_t Helper Tests
 * ============================================================================ */

TEST(buf_descriptor_unregistered) {
    char data[64];
    auraio_buf_t b = auraio_buf(data);
    assert(b.type == AURAIO_BUF_UNREGISTERED);
    assert(b.u.ptr == data);
}

TEST(buf_descriptor_fixed) {
    auraio_buf_t b = auraio_buf_fixed(3, 512);
    assert(b.type == AURAIO_BUF_REGISTERED);
    assert(b.u.fixed.index == 3);
    assert(b.u.fixed.offset == 512);
}

TEST(buf_descriptor_fixed_idx) {
    auraio_buf_t b = auraio_buf_fixed_idx(7);
    assert(b.type == AURAIO_BUF_REGISTERED);
...
TEST(buf_descriptor_negative_index) {
    auraio_buf_t b = auraio_buf_fixed(-1, 0);
    assert(b.type == AURAIO_BUF_UNREGISTERED);
    assert(b.u.ptr == NULL);
}
