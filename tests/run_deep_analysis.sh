...
build_debug() {
    ...
    local DEBUG_CFLAGS="-Wall -Wextra -std=c11 -O2 -fPIC -Iengine/include -Iengine/src -g -fno-omit-frame-pointer"
    make clean > /dev/null 2>&1
    make -j"$(nproc)" all CFLAGS="$DEBUG_CFLAGS" > /dev/null 2>&1

    # Rebuild perf_bench with same flags
    local BENCH_CFLAGS="-Wall -Wextra -std=c11 -O2 -g -fno-omit-frame-pointer -I../engine/include -I../engine/src"
