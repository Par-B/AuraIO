...
#ifndef AURA_DETAIL_CALLBACK_STORAGE_HPP
#define AURA_DETAIL_CALLBACK_STORAGE_HPP
...
class CallbackPool {
  public:
    ...
    explicit CallbackPool(aura_engine_t *engine, size_t initial_size_per_shard = 16)
        : engine_(engine) {
 ...
 * pointer (aura_callback_t) to the C API. Calling a C++ function through
 ...
 */
extern "C" inline void aura_detail_callback_trampoline(aura_request_t *req, ssize_t result,
                                                         void *user_data) {
    auto *ctx = static_cast<aura::detail::CallbackContext *>(user_data);
    if (ctx && ctx->callback) {
        ctx->request = aura::Request(req);
        try {
...
#endif // AURA_DETAIL_CALLBACK_STORAGE_HPP
