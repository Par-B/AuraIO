...
#ifndef AURA_BUFFER_HPP
#define AURA_BUFFER_HPP
 ...
 *
 * Wraps aura_buf_t for passing to I/O operations.
...
class BufferRef {
  public:
     ...
     */
    BufferRef(void *ptr) noexcept : buf_(aura_buf(ptr)) {}
     ...
     */
    explicit BufferRef(const void *ptr) noexcept : buf_(aura_buf(const_cast<void *>(ptr))) {}
    ...
    [[nodiscard]] static BufferRef fixed(int index, size_t offset = 0) noexcept {
        BufferRef ref;
        ref.buf_ = aura_buf_fixed(index, offset);
        return ref;
    }
     ...
     */
    [[nodiscard]] bool is_registered() const noexcept { return buf_.type == AURA_BUF_REGISTERED; }

    /**
     * Get underlying C buffer descriptor
     * @return aura_buf_t value
     */
    [[nodiscard]] aura_buf_t c_buf() const noexcept { return buf_; }
...
class Buffer {
  ...
  private:
    ...
    Buffer(aura_engine_t *engine, std::shared_ptr<std::atomic<bool>> engine_alive, void *ptr,
...
// Implementation of release_internal (needs aura.h)
