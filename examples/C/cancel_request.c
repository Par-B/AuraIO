 ...
 *
 * Shows how to use auraio_cancel() to abort a pending read.
...
static void on_read(auraio_request_t *req, ssize_t result, void *user_data) {
    (void)req;
    (void)user_data;
    read_result = result;
    completed = 1;
...
int main(int argc, char *argv[]) {
    ...
    auraio_request_t *req = auraio_read(engine, fd, auraio_buf(buf), READ_SIZE, 0,
                                         on_read, NULL);
    if (!req) {
    ...
    int cancel_ret = auraio_cancel(engine, req);
    if (cancel_ret == 0) {
