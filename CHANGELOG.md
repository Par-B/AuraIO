...
- **Code quality improvements** (no functional changes):
  ...
  - C engine: Consolidated 6 lifecycle metadata operations (`ring_submit_openat`, `ring_submit_close`, `ring_submit_statx`, `ring_submit_fallocate`, `ring_submit_ftruncate`, `ring_submit_sync_file_range`) using `RING_SUBMIT_META_OP` and `RING_SUBMIT_META_OP_FIXED` macros, reducing boilerplate by ~42 lines (60→18 lines). Preserves special handling: close's no-fixed-file semantics, ftruncate's ENOSYS guard, sync_file_range's UINT_MAX validation.
