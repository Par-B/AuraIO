...
TARGET_VALGRIND_VERSION="${AURAIO_VALGRIND_TARGET_VERSION:-3.26.0}"
BUILD_VALGRIND_FROM_SOURCE="${AURAIO_BUILD_VALGRIND_FROM_SOURCE:-1}"
...
if [ -z "$VALGRIND_VER" ]; then
    ...
    if [ "$BUILD_VALGRIND_FROM_SOURCE" = "1" ]; then
        ...
        TMP_DIR="/tmp/auraio-valgrind-build"
        rm -rf "$TMP_DIR"
        mkdir -p "$TMP_DIR"
        cd "$TMP_DIR"
