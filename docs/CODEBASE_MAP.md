---
last_mapped: 2026-02-09T03:37:56Z
total_files: 106
total_tokens: 282207
---
...
- Logging: `auraio_set_log_handler()` (new in latest version)
...
| `auraio_prometheus.c` | Text format implementation | 2,576 |
...
| File | Purpose | Tokens |
|------|---------|--------|
| `auraio_otel.h` | Formatter API, schema version constants | 438 |
| `auraio_otel.c` | OTLP JSON format implementation | 3,662 |
| `auraio_otel_push.h` | HTTP push helper API | 290 |
| `auraio_otel_push.c` | Blocking HTTP POST to OTel collector | 1,651 |
...
- `auraio_request_t` - Request with callback, timing, `uses_registered_buffer`/`uses_registered_file` flags (128-byte cache-line aligned)
...
- **`auraio_log()`**: Emit log message (no-op if no handler)
- **Log levels**: `AURAIO_LOG_ERR=3`, `AURAIO_LOG_WARN=4` (match syslog)

**Key Patterns**:
- **Atomic handler storage**: `_Atomic(auraio_log_fn)` + `memory_order_seq_cst` for cross-variable ordering
...
## Recent Critical Fixes (since last map)
...
| Log handler | `_Atomic(auraio_log_fn)` + seq_cst | Process-wide, thread-safe |
