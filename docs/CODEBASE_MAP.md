...
| File | Purpose | Tokens |
|------|---------|--------|
| `include/auraio.h` | Public types and function declarations | 8,286 |
| `src/auraio.c` | API implementation, ring selection modes, merged flush+poll | 14,111 |
...
**Registered Buffer Protection**: `pthread_rwlock_t reg_lock` serializes buffer/file registration with in-flight I/O. Read lock held during submission validation; write lock during register/unregister. Overflow-safe bounds checking: `offset >= iov_len || len > iov_len - offset`. `auraio_destroy()` drains all pending I/O **before** unregistering buffers.
...
| `include/auraio/engine.hpp` | Engine class, callbacks, concepts, per-ring stats, deferred unregister | 4,646 |
| `include/auraio/coro.hpp` | Task<T>, IoAwaitable, FsyncAwaitable | 3,204 |
| `include/auraio/buffer.hpp` | Buffer (RAII), BufferRef (explicit const ctor), engine lifetime tracking | 2,521 |
...
- `engine_alive_` shared atomic flag: buffers safely outlive engine (fallback to `free()`)
...
| File | Purpose | Tokens |
|------|---------|--------|
| `auraio-sys/build.rs` | Library detection + bindgen codegen | 778 |
| `auraio-sys/src/lib.rs` | Raw FFI re-exports | 450 |
| `auraio/src/lib.rs` | Public API, integration tests | 8,907 |
| `auraio/src/engine.rs` | Engine RAII wrapper, all I/O methods, deferred unregister | 4,815 |
| `auraio/src/async_io.rs` | AsyncEngine trait, IoFuture | 4,155 |
...
- `poll_lock: Mutex<()>` serializes event loop operations (prevents concurrent CQ access)
- Errno captured **before** `drop()` in error paths (drop could clobber errno)
...
| File | Purpose | Tokens |
|------|---------|--------|
| `auraio_prometheus.h` | Formatter API, schema version constants | 403 |
| `auraio_prometheus.c` | Text format implementation | 2,434 |
...
| File | Purpose | Tokens |
|------|---------|--------|
| `src/adaptive_ring.h` | Ring interface, request struct, latency sampling config | 2,279 |
| `src/adaptive_ring.c` | Ring implementation, COOP_TASKRUN, batched CQE extraction, callback context tracking | 6,848 |
...
- `ring_in_callback_context()` - TLS-based callback context detection
...
| `src/adaptive_engine.c` | AIMD implementation, low-IOPS handling | 3,938 |
...
| File | Purpose | Tokens |
|------|---------|--------|
| `src/adaptive_buffer.h` | Pool interface | 1,416 |
| `src/adaptive_buffer.c` | Pool implementation, TLS caching, CAS registration | 5,880 |
...
- TLS destructor coordination via `cleanup_mutex` (ensures exactly one side handles buffers, exactly one side frees struct)
- Pool ID generation via global atomic counter (detects stale caches from reused addresses)
...
| Fixed buf inflight | `_Atomic uint32_t` per ring | Track in-flight registered buffer ops for safe unregister |
| C++ event loop | `std::mutex` | Serializes poll/wait/run; submissions lock-free |
