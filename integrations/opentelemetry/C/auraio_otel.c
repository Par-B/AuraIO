...
int auraio_metrics_otel(auraio_engine_t *engine, char *buf, size_t buf_size) {
    ...
    OTEL_APPEND("{\"resourceMetrics\":[{\"resource\":{\"attributes\":["
                "{\"key\":\"service.name\",\"value\":{\"stringValue\":\"auraio\"}},"
                "{\"key\":\"auraio.schema.version\",\"value\":{\"stringValue\":"
                "\"" AURAIO_OTEL_SCHEMA_VERSION "\"}},"
                "{\"key\":\"auraio.schema.stability\",\"value\":{\"stringValue\":"
                "\"" AURAIO_OTEL_SCHEMA_STABILITY "\"}}"
                "]},\"scopeMetrics\":[{\"scope\":{\"name\":\"auraio.engine\",\"version\":\"%s\"},"
                "\"metrics\":[",
                auraio_version());
    ...
    /* ops_completed */
    OTEL_APPEND("{\"name\":\"auraio.ops.completed\",\"unit\":\"{operation}\","
    ...
    /* bytes_transferred */
    OTEL_APPEND(",{\"name\":\"auraio.bytes.transferred\",\"unit\":\"By\","
                "\"sum\":{\"aggregationTemporality\":2,\"isMonotonic\":true,"
                "\"dataPoints\":[{\"asInt\":\"%" PRId64 "\","
                "\"startTimeUnixNano\":\"0\",\"timeUnixNano\":\"%s\"}]}}",
    ...
    /* adaptive_spills */
    OTEL_APPEND(",{\"name\":\"auraio.adaptive.spills\",\"unit\":\"{operation}\","
                "\"sum\":{\"aggregationTemporality\":2,\"isMonotonic\":true,"
    ...
    OTEL_APPEND(",{\"name\":\"auraio.buffer_pool.allocated\",\"unit\":\"By\","
                "\"gauge\":{\"dataPoints\":[{\"asInt\":\"%zu\","
                "\"timeUnixNano\":\"%s\"}]}}",
                bstats.total_allocated_bytes, time_str);

    OTEL_APPEND(",{\"name\":\"auraio.buffer_pool.buffers\",\"unit\":\"{buffer}\","
                "\"gauge\":{\"dataPoints\":[{\"asInt\":\"%zu\","
                "\"timeUnixNano\":\"%s\"}]}}",
                bstats.total_buffers, time_str);

    OTEL_APPEND(",{\"name\":\"auraio.buffer_pool.shards\",\"unit\":\"{shard}\","
                "\"gauge\":{\"dataPoints\":[{\"asInt\":\"%d\","
                "\"timeUnixNano\":\"%s\"}]}}",
    ...
    if (ring_count > 0) {
        ...
        OTEL_APPEND(",{\"name\":\"auraio.ring.ops.completed\",\"unit\":\"{operation}\","
                    "\"sum\":{\"aggregationTemporality\":2,\"isMonotonic\":true,"
                    "\"dataPoints\":[");
        for (int i = 0; i < ring_count; i++) {
            if (i > 0) OTEL_APPEND(",");
        ...
        OTEL_APPEND(",{\"name\":\"auraio.ring.bytes.transferred\",\"unit\":\"By\","
                    "\"sum\":{\"aggregationTemporality\":2,\"isMonotonic\":true,"
                    "\"dataPoints\":[");
        for (int i = 0; i < ring_count; i++) {
            if (i > 0) OTEL_APPEND(",");
        ...
        OTEL_APPEND(",{\"name\":\"auraio.ring.in_flight\",\"unit\":\"{operation}\","
                    "\"gauge\":{\"dataPoints\":[");
        for (int i = 0; i < ring_count; i++) {
            if (i > 0) OTEL_APPEND(",");
        ...
        OTEL_APPEND(",{\"name\":\"auraio.ring.in_flight_limit\",\"unit\":\"{operation}\","
                    "\"gauge\":{\"dataPoints\":[");
        for (int i = 0; i < ring_count; i++) {
            if (i > 0) OTEL_APPEND(",");
            OTEL_APPEND("{\"asInt\":\"%d\",\"timeUnixNano\":\"%s\","
        ...
        OTEL_APPEND(",{\"name\":\"auraio.ring.batch_threshold\",\"unit\":\"{operation}\","
                    "\"gauge\":{\"dataPoints\":[");
        for (int i = 0; i < ring_count; i++) {
            if (i > 0) OTEL_APPEND(",");
            OTEL_APPEND("{\"asInt\":\"%d\",\"timeUnixNano\":\"%s\","
        ...
        OTEL_APPEND(",{\"name\":\"auraio.ring.queue_depth\",\"unit\":\"{operation}\","
                    "\"gauge\":{\"dataPoints\":[");
        for (int i = 0; i < ring_count; i++) {
            if (i > 0) OTEL_APPEND(",");
            OTEL_APPEND("{\"asInt\":\"%d\",\"timeUnixNano\":\"%s\","
        ...
        OTEL_APPEND(",{\"name\":\"auraio.ring.p99_latency\",\"unit\":\"s\","
                    "\"gauge\":{\"dataPoints\":[");
        for (int i = 0; i < ring_count; i++) {
            if (i > 0) OTEL_APPEND(",");
        ...
        OTEL_APPEND(",{\"name\":\"auraio.ring.throughput\",\"unit\":\"By/s\","
                    "\"gauge\":{\"dataPoints\":[");
        for (int i = 0; i < ring_count; i++) {
            if (i > 0) OTEL_APPEND(",");
        ...
        OTEL_APPEND(",{\"name\":\"auraio.ring.aimd_phase\",\"unit\":\"{phase}\","
                    "\"gauge\":{\"dataPoints\":[");
        for (int i = 0; i < ring_count; i++) {
            if (i > 0) OTEL_APPEND(",");
            OTEL_APPEND("{\"asInt\":\"%d\",\"timeUnixNano\":\"%s\","
        ...
        OTEL_APPEND(",{\"name\":\"auraio.latency\",\"unit\":\"s\","
        ...
        for (int i = 0; i < ring_count; i++) {
             ...
             * Boundaries are the upper edge of each of the AURAIO_HISTOGRAM_BUCKETS buckets. */
            ...
            /* bucketCounts: AURAIO_HISTOGRAM_BUCKETS + 1 (last is overflow) */
