...
int auraio_metrics_prometheus(auraio_engine_t *engine, char *buf, size_t buf_size) {
    ...
    PROM_APPEND("# HELP auraio_metrics_schema_info Exporter metrics schema and stability\n"
    ...
    PROM_APPEND("# HELP auraio_ops_completed_total Total I/O operations completed\n"
                "# TYPE auraio_ops_completed_total counter\n"
                "auraio_ops_completed_total %" PRId64 "\n\n",
                stats.ops_completed);

    PROM_APPEND("# HELP auraio_bytes_transferred_total Total bytes transferred\n"
                "# TYPE auraio_bytes_transferred_total counter\n"
                "auraio_bytes_transferred_total %" PRId64 "\n\n",
                stats.bytes_transferred);

    PROM_APPEND("# HELP auraio_throughput_bytes_per_second Current aggregate throughput\n"
    ...
    PROM_APPEND("# HELP auraio_p99_latency_seconds Aggregate P99 latency\n"
                "# TYPE auraio_p99_latency_seconds gauge\n"
                "auraio_p99_latency_seconds %.6f\n\n",
                prom_finite(stats.p99_latency_ms / 1000.0));

    PROM_APPEND("# HELP auraio_in_flight Current total in-flight operations\n"
                "# TYPE auraio_in_flight gauge\n"
                "auraio_in_flight %d\n\n",
                stats.current_in_flight);

    PROM_APPEND("# HELP auraio_optimal_in_flight Aggregate AIMD-tuned in-flight limit\n"
                "# TYPE auraio_optimal_in_flight gauge\n"
                "auraio_optimal_in_flight %d\n\n",
                stats.optimal_in_flight);

    PROM_APPEND("# HELP auraio_optimal_batch_size Aggregate AIMD-tuned batch size\n"
                "# TYPE auraio_optimal_batch_size gauge\n"
                "auraio_optimal_batch_size %d\n\n",
                stats.optimal_batch_size);

    PROM_APPEND(
        "# HELP auraio_adaptive_spills_total ADAPTIVE mode: submissions spilled to non-local ring\n"
        "# TYPE auraio_adaptive_spills_total counter\n"
        "auraio_adaptive_spills_total %" PRIu64 "\n\n",
        stats.adaptive_spills);

    PROM_APPEND("# HELP auraio_ring_count Number of io_uring rings\n"
    ...
    if (ring_count > 0) {
        ...
        PROM_APPEND("# HELP auraio_ring_ops_completed_total Operations completed per ring\n"
        ...
        PROM_APPEND("# HELP auraio_ring_bytes_transferred_total Bytes transferred per ring\n"
        ...
        PROM_APPEND("# HELP auraio_ring_in_flight Current in-flight operations per ring\n"
        ...
        PROM_APPEND("# HELP auraio_ring_in_flight_limit AIMD-tuned in-flight limit per ring\n"
        ...
        PROM_APPEND("# HELP auraio_ring_batch_threshold AIMD-tuned batch threshold per ring\n"
        ...
        PROM_APPEND("# HELP auraio_ring_queue_depth Kernel queue depth per ring\n"
        ...
        PROM_APPEND("# HELP auraio_ring_p99_latency_seconds P99 latency per ring\n"
        ...
        PROM_APPEND("# HELP auraio_ring_throughput_bytes_per_second Throughput per ring\n"
        ...
        PROM_APPEND("# HELP auraio_ring_aimd_phase Current AIMD phase per ring "
        ...
        PROM_APPEND("# HELP auraio_latency_seconds Latency distribution per ring "
        ...
        for (int i = 0; i < ring_count; i++) {
            ...
            for (int b = 0; b < AURAIO_HISTOGRAM_BUCKETS; b++) {
                ...
                if (cumulative > 0) {
                    ...
                    PROM_APPEND("auraio_latency_seconds_bucket{ring=\"%d\",le=\"%.6f\"} %" PRIu64
                                "\n",
                                i, le, cumulative);
                }
            }
            ...
            PROM_APPEND("auraio_latency_seconds_bucket{ring=\"%d\",le=\"+Inf\"} %" PRIu64 "\n", i,
                        cumulative);
            PROM_APPEND("auraio_latency_seconds_sum{ring=\"%d\"} %.6f\n", i, sum_estimate);
            /* Use cumulative (computed from buckets + overflow) as _count
             * to guarantee _count == le="+Inf" per Prometheus spec. */
            PROM_APPEND("auraio_latency_seconds_count{ring=\"%d\"} %" PRIu64 "\n", i, cumulative);
        }
        PROM_APPEND("\n");
    }
    ...
    PROM_APPEND("# HELP auraio_buffer_pool_allocated_bytes Total bytes allocated from buffer pool\n"
                "# TYPE auraio_buffer_pool_allocated_bytes gauge\n"
                "auraio_buffer_pool_allocated_bytes %zu\n\n",
                bstats.total_allocated_bytes);

    PROM_APPEND("# HELP auraio_buffer_pool_buffers Total buffers allocated from pool\n"
                "# TYPE auraio_buffer_pool_buffers gauge\n"
                "auraio_buffer_pool_buffers %zu\n\n",
                bstats.total_buffers);

    PROM_APPEND("# HELP auraio_buffer_pool_shards Number of buffer pool shards\n"
