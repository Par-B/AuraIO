...
static void print_percentiles(const direction_result_t *r, FILE *out)
{
    ...
    static const char *pct_labels[NUM_PERCENTILES] = {
 ...
 */
static void print_direction_normal(const direction_result_t *r,
                                   const char *name, FILE *out)
{
    ...
    fprintf(out, "  %s: IOPS=%s, BW=%s(%s/%" PRId64 "msec)\n",
            name, iops_buf, bw_buf, size_buf, r->runtime_ms);
    ...
    fprintf(out, "    lat (%s): min=%.0f, max=%.0f, avg=%.2f, stdev=%.2f\n",
            lat_unit, lat_min, lat_max, lat_avg, lat_dev);

    /* Clat percentiles */
    print_percentiles(r, out);

    /* BW sample stats */
    fprintf(out, "   bw (  KiB/s): min=%" PRId64 ", max=%" PRId64
            ", per=100.00%%, avg=%.2f, stdev=%.2f, samples=%d\n",
            r->bw_min, r->bw_max, r->bw_mean, r->bw_dev, r->bw_samples);

    /* IOPS sample stats */
    fprintf(out, "   iops        : min=%" PRId64 ", max=%" PRId64
            ", avg=%.2f, stdev=%.2f, samples=%d\n",
            r->iops_min, r->iops_max, r->iops_mean, r->iops_stddev,
            r->iops_samples);
}
                   ...
                   const bench_config_t *config, FILE *out)
{
    for (int j = 0; j < num_jobs; j++) {
        ...
        if (job->rw == RW_READWRITE || job->rw == RW_RANDRW) {
            fprintf(out, "%s: (g=0): rw=%s, bs=(R) %" PRIu64
                    "B-(W) %" PRIu64 "B, ioengine=AuraIO, direct=%d\n",
                    r->jobname, rw_pattern_name(job->rw),
                    job->bs, job->bs, job->direct);
        } else {
            fprintf(out, "%s: (g=0): rw=%s, bs=(R) %" PRIu64
        ...
        if (r->target_p99_ms > 0.0) {
            fprintf(out, "  AuraIO: max concurrency=%d at p99=%.2fms"
                    " (target: %.2fms), phase=%s\n",
                    r->auraio_final_depth, r->auraio_p99_ms,
                    r->target_p99_ms, r->auraio_phase_name);
        } else {
            fprintf(out, "  AuraIO: converged to depth %d, phase=%s,"
                    " p99=%.2fms\n",
                    r->auraio_final_depth, r->auraio_phase_name,
                    r->auraio_p99_ms);
        }
    }
}
 ...
 */
static void json_lat_ns(FILE *out, const char *indent,
                         uint64_t min_ns, uint64_t max_ns,
                         double mean_ns, double stddev_ns,
                         uint64_t count,
                         const uint64_t *percentiles_ns,
                         int include_percentiles)
{
    fprintf(out, "{\n");
    ...
    if (include_percentiles && percentiles_ns != NULL && count > 0) {
        ...
        static const char *pct_keys[NUM_PERCENTILES] = {
            "1.000000",  "5.000000",  "10.000000", "20.000000",
            "30.000000", "40.000000", "50.000000", "60.000000",
            "70.000000", "80.000000", "90.000000", "95.000000",
            "99.000000", "99.500000", "99.900000", "99.950000",
            "99.990000"
        };

        for (int i = 0; i < NUM_PERCENTILES; i++) {
            fprintf(out, "%s    \"%s\" : %" PRIu64,
                    indent, pct_keys[i], percentiles_ns[i]);
            if (i < NUM_PERCENTILES - 1) {
                fprintf(out, ",\n");
            } else {
                fprintf(out, "\n");
 ...
 */
static void json_direction(FILE *out, const direction_result_t *r,
                           const char *indent)
{
    ...
    json_lat_ns(out, inner, r->lat_min_ns, r->lat_max_ns,
                r->lat_mean_ns, r->lat_stddev_ns, r->lat_count,
                r->lat_percentiles_ns, 1);
    fprintf(out, ",\n");
 ...
 */
static void json_job_options(FILE *out, const job_config_t *job,
                             const char *indent)
{
    char bs_buf[32];
    char size_buf[32];
    ...
    if (job->bs >= 1024 * 1024) {
        snprintf(bs_buf, sizeof(bs_buf), "%" PRIu64 "m",
                 job->bs / (1024 * 1024));
    } else if (job->bs >= 1024) {
    ...
    if (job->size >= (uint64_t)1024 * 1024 * 1024) {
        ...
        snprintf(size_buf, sizeof(size_buf), "%" PRIu64 "M",
                 job->size / ((uint64_t)1024 * 1024));
    } else if (job->size >= 1024) {
        snprintf(size_buf, sizeof(size_buf), "%" PRIu64 "K",
                 job->size / 1024);
    } else {
    ...
    if (job->directory[0] != '\0') {
        fprintf(out, ",\n%s  \"directory\" : \"%s\"", indent, job->directory);
    }
    if (job->filename[0] != '\0') {
        fprintf(out, ",\n%s  \"filename\" : \"%s\"", indent, job->filename);
    }
