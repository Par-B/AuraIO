...
{
    printf(
        "BFFIO - Better Faster FIO (powered by AuraIO)\n"
        "\n"
        "Usage:\n"
        ...
        "  --rw=PATTERN           I/O pattern: read, write, randread, randwrite, readwrite, randrw\n"
        ...
        "  --iodepth=N            Max queue depth cap (default: 256, AuraIO auto-tunes below this)\n"
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < bench.num_jobs; i++) {
        ...
        thread_stats_t *thread_stats = calloc((size_t)job->numjobs,
                                              sizeof(thread_stats_t));
        if (!thread_stats) {
        ...
        for (int t = 0; t < job->numjobs; t++)
            stats_init(&thread_stats[t]);
        ...
        if (job->group_reporting && job->numjobs > 1) {
            ...
            stats_compute_results(&aggregated, runtime_ms, engine,
                                  job->name, &results[i]);
        } else {
            stats_compute_results(&thread_stats[0], runtime_ms, engine,
                                  job->name, &results[i]);
        }
