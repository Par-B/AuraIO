...
const double PERCENTILE_LIST[NUM_PERCENTILES] = { 1.0,  5.0,  10.0, 20.0,  30.0, 40.0,
...
void stats_reset(thread_stats_t *s) {
    ...
    /* Note: inflight is intentionally NOT reset — it tracks live
     * in-flight I/Os and zeroing it would misrepresent the actual count. */
...
void stats_record_io(thread_stats_t *s, uint64_t latency_ns, size_t bytes, int is_write) {
    ...
    while (latency_ns < cur_min) {
        if (atomic_compare_exchange_weak_explicit(&s->lat_min_ns, &cur_min, latency_ns,
    ...
    while (latency_ns > cur_max) {
        if (atomic_compare_exchange_weak_explicit(&s->lat_max_ns, &cur_max, latency_ns,
                                                  memory_order_relaxed, memory_order_relaxed)) {
            break;
        }
    }
...
void stats_aggregate(const thread_stats_t *per_thread, int num_threads, thread_stats_t *out) {
    ...
    for (int t = 0; t < num_threads; t++) {
        ...
        /* Merge histograms */
        for (int i = 0; i < LAT_HIST_BUCKETS; i++) {
            uint64_t count = atomic_load_explicit(&src->lat_hist[i], memory_order_relaxed);
            atomic_fetch_add_explicit(&out->lat_hist[i], count, memory_order_relaxed);
        }
        uint64_t overflow = atomic_load_explicit(&src->lat_overflow, memory_order_relaxed);
        atomic_fetch_add_explicit(&out->lat_overflow, overflow, memory_order_relaxed);

        /* Min of mins */
        uint64_t src_min = atomic_load_explicit(&src->lat_min_ns, memory_order_relaxed);
        uint64_t cur_min = atomic_load_explicit(&out->lat_min_ns, memory_order_relaxed);
        while (src_min < cur_min) {
            if (atomic_compare_exchange_weak_explicit(&out->lat_min_ns, &cur_min, src_min,
                                                      memory_order_relaxed, memory_order_relaxed)) {
                break;
            }
        }
        ...
        /* Max of maxes */
        uint64_t src_max = atomic_load_explicit(&src->lat_max_ns, memory_order_relaxed);
        uint64_t cur_max = atomic_load_explicit(&out->lat_max_ns, memory_order_relaxed);
        while (src_max > cur_max) {
            if (atomic_compare_exchange_weak_explicit(&out->lat_max_ns, &cur_max, src_max,
                                                      memory_order_relaxed, memory_order_relaxed)) {
                break;
            }
        }
        ...
        uint64_t lsum = atomic_load_explicit(&src->lat_sum_ns, memory_order_relaxed);
        uint64_t lcnt = atomic_load_explicit(&src->lat_count, memory_order_relaxed);
...
uint64_t stats_percentile(const thread_stats_t *s, double pct) {
    ...
    uint64_t overflow = atomic_load_explicit(&s->lat_overflow, memory_order_relaxed);
    ...
    for (int i = 0; i < LAT_HIST_BUCKETS; i++) {
        cumulative += atomic_load_explicit(&s->lat_hist[i], memory_order_relaxed);
...
static void compute_sample_stats(const uint64_t *samples, int n, int64_t *out_min, int64_t *out_max,
...
static double compute_lat_stddev_from_hist(const thread_stats_t *s, double mean_ns) {
    /*
    ...
    for (int i = 0; i < LAT_HIST_BUCKETS; i++) {
        uint64_t count = atomic_load_explicit(&s->lat_hist[i], memory_order_relaxed);
        if (count == 0) {
            continue;
        }
...
static void compute_direction(const thread_stats_t *raw, uint64_t runtime_ms, int is_write,
                              direction_result_t *dir) {
    ...
    uint64_t lat_count = atomic_load_explicit(&raw->lat_count, memory_order_relaxed);
    dir->lat_count = lat_count;

    if (lat_count > 0) {
        uint64_t lat_min = atomic_load_explicit(&raw->lat_min_ns, memory_order_relaxed);
        uint64_t lat_max = atomic_load_explicit(&raw->lat_max_ns, memory_order_relaxed);
        uint64_t lat_sum = atomic_load_explicit(&raw->lat_sum_ns, memory_order_relaxed);

        dir->lat_min_ns = (lat_min == UINT64_MAX) ? 0 : lat_min;
        dir->lat_max_ns = lat_max;
        dir->lat_mean_ns = (double)lat_sum / (double)lat_count;
        dir->lat_stddev_ns = compute_lat_stddev_from_hist(raw, dir->lat_mean_ns);

        /* Compute all 17 percentiles */
        for (int i = 0; i < NUM_PERCENTILES; i++) {
            dir->lat_percentiles_ns[i] = stats_percentile(raw, PERCENTILE_LIST[i]);
        }
    }
}
...
void stats_compute_results(const thread_stats_t *raw, uint64_t runtime_ms, aura_engine_t *engine,
    ...
    if (raw->sample_count > 0) {
        ...
        compute_sample_stats(bw_kib, raw->sample_count, &bw_min, &bw_max, &bw_mean, &bw_dev);

        /* Apply to whichever direction had I/O (or both) */
        uint64_t rops = atomic_load_explicit(&raw->read_ops, memory_order_relaxed);
        uint64_t wops = atomic_load_explicit(&raw->write_ops, memory_order_relaxed);
        ...
        compute_sample_stats(raw->iops_samples, raw->sample_count, &iops_min, &iops_max, &iops_mean,
                             &iops_stddev);
